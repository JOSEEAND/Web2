...
export class DetalleFacturaController{
    ...
    static getById=async(req:Request,res:Response)=>{
        ...
        try {
            ...
            const detallesRepo=AppDataSource.getRepository(DetalleFactura);
            const detalles=await detallesRepo.findOne({where:{idDetalleFactura}});
    ...
    static add=async(req:Request,res:Response)=>{
        ...
        try {
            const {idDetalleFactura,idFactura,cantidad,codigoProducto}=req.body;
            ...
            if(!factura || !producto){
                return res.status(404).json({message:'No se encuentra la factura o producto'});
            }
    ...
    static update=async(req:Request,res:Response)=>{
        ...
        try {
            ...
            if(!factura || !producto){
                return res.status(404).json({message:'No se encuentran la factura o producto'});
            }

            try {
                detalle=await detallesRepo.findOneOrFail({where:{idDetalleFactura}});
            } catch (error) {
