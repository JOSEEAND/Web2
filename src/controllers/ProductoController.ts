...
export class ProductoController{
    ...
    static update=async(req:Request,res:Response)=>{
        try {
            const codigo=parseInt(req.params['codigo']);
            const {descripcion,precio,stockMaximo,stockMinimo,codigoProveedor}=req.body;

            if(!codigo || !descripcion || !stockMaximo || !stockMinimo
                || !codigoProveedor){
                return res.status(404).json({mensaje:'Debe ingresar valores validos'});
            }
